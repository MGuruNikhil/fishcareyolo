      ...
      <div className="flex flex-row gap-4 mt-2">
        {primaryButtonLabel && (
          <Button asChild={!!primaryButtonLink} size="lg">
            {primaryButtonLink ? <a href={primaryButtonLink}>{primaryButtonLabel}</a> : primaryButtonLabel}
          </Button>
        )}
        {secondaryButtonLabel && (
          <Button asChild={!!secondaryButtonLink} variant="outline" size="lg">
            {secondaryButtonLink ? <a href={secondaryButtonLink}>{secondaryButtonLabel}</a> : secondaryButtonLabel}
          </Button>
        )}
      </div>
      {showSignupCard && (
        <Card className="w-full max-w-lg mt-6">
          <CardHeader>{signupCardTitle && <CardTitle>{signupCardTitle}</CardTitle>}</CardHeader>
          <CardContent>
    ...
    <section className="container mx-auto py-10 px-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {features.map(({ title, description, icon }, index) => {
          const IconComponent =
            (LucideIcons[icon] as React.ElementType) || (LucideIcons.CircleDashed as React.ElementType)
