...
const Navbar: React.FC<NavbarProps> = ({ navItems = {} }) => {
  ...
  const renderNavItems = (items: NavItem[], variant: "desktop" | "mobile" = "desktop") =>
      ...
      return (
        ...
        >
          <a href={item.href}>
            ...
            {item.label && <span className={isMobile ? "" : "text-sm font-medium"}>{item.label}</span>}
          </a>
        </Button>
      )
    })
  ...
  return (
    <>
      ...
      <header className="hidden lg:flex items-center justify-between px-4 py-3 bg-background border-b border-border sticky top-0 left-0 w-full">
        <div className="flex items-center gap-4">{renderNavItems(left, "desktop")}</div>
        {center.length > 0 && <nav className="flex items-center gap-6">{renderNavItems(center, "desktop")}</nav>}
        <div className="flex items-center gap-4">{renderNavItems(right, "desktop")}</div>
      </header>
      ...
      {center.length > 0 && (
        <nav className="lg:hidden fixed bottom-0 left-0 w-full bg-background py-2 border-t border-border">
          <div className="flex items-center justify-around">{renderNavItems(center, "mobile")}</div>
        </nav>
      )}
    </>
  )
